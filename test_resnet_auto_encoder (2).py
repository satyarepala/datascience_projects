...
class CNNAutoencoder(nn.Module):
    def __init__(self, bottleneck_size=512):
        ...
        # Encoder: progressively reduce spatial dimensions
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),  # Output: (16, 4, 50)
            nn.ReLU(inplace=True),

            nn.Conv2d(16, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),  # Output: (32, 2, 25)
            nn.ReLU(inplace=True),

            nn.Conv2d(32, 64, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))   # Output: (64, 1, 13)
        )

        # Add a fully connected layer to transform the output to a 1D vector
        self.flatten = nn.Flatten()
        self.fc = nn.Linear(64 * 1 * 13, bottleneck_size)  # 64 channels, 1x13 spatial dimensions

        # Decoder: progressively reconstruct spatial dimensions
        self.decoder = nn.Sequential(
            nn.Linear(bottleneck_size, 64 * 1 * 13),  # Transform back to the shape needed for decoding
            nn.Unflatten(1, (64, 1, 13)),

            nn.ConvTranspose2d(64, 32, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1), output_padding=(0, 0)),  # Output: (32, 2, 26)
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(32, 16, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), output_padding=(0, 0)),  # Output: (16, 4, 52)
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(16, 8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), output_padding=(0, 0)),   # Output: (8, 7, 104)
            nn.ReLU(inplace=True),

            nn.ConvTranspose2d(8, 1, kernel_size=(3, 2), stride=(1, 1), padding=(0, 1), output_padding=(0, 0))    # Output: (1, 7, 100)
        )
...
# Example input
input_tensor = torch.randn(1, 1, 7, 100)  # Batch size of 1, 1 channel, 7x100 spatial dimensions
output_tensor = model(input_tensor)
...
x_data = np.random.randn(num_samples, 1,num_channels, seq_length).astype(np.float32)
